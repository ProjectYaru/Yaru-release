...
- 繁体中文文档：[`docs/zh_Hant/`](./docs/zh_Hant/)

## 更新元数据与发布流程

Yaru 的发布元数据以 `latest.json` 为中心：

- `latest.json` 记录当前版本、发布时间、最低支持版本，以及各平台下载地址、校验值和体积信息。
- 主 Yaru Monorepo 的发布流程会构建平台产物，上传到本仓库的 GitHub Release，再把平台元数据汇总进 `latest.json`，并对照已上传的 Release 资产进行校验。
- 当 Release 资产与远端 `latest.json` 都验证通过后，发布流程还会访问 <https://yaru.asaka.moe/download/>，作为公开下载页预热与发布成功路径的一部分。
